...
The plot demonstrates that ALbuquerque, NM, Stockton, CA, and Fresno, CA have the highest odds of solving homicides with male victims compared to female victims. Of note, the 95% confidence intervals for these cities is extremely wide, especially compared to the rest of the data set. Meanwhile, New York, NY, Baton Rouge, LA, and Omaha, NE have the lowest odds of solving homicides with male victims compared to female victims, and they have tighter confidence intervals. 

## Problem 3

### Load and Clean Data for Regression Analysis

Four variables (`babysex`, `frace`, `malform`, `mrace`) were converted to factors since they are categorical. `frace` had no "unknown" responses and `mrace` had no "other" responses, therefore, neither of those responses were included in the factor recode. `fincome` was also modified to reflect the appropriate unit reported in the codebook (hundreds). There is no other missing data for any other variables in this data set. The `skimr` function provides a summary of the variables in the data set below.
...
We hypothesize that birth weight (grams) is associated with the mother's weight at the time of delivery (pounds).
...
To test this hypothesis, we will use a regression model using birth weight and the baby's length at birth. Below, predictions (x) and residuals (y) are used to illustrate the model.
...
* One using head circumference, length, sex, and all interactions as predictors
...
Using cross-validated prediction error, we are able to make comparisons between the three models (`mod_blength`, `mod_blength_gaweeks`, and `mod_bhead_blength_bsex`). 
...
The plot below demonstrates the distribution of RMSE values for each model. RMSE is a metric that ranges from 0 to infinity, and a lower value represents a "better fitted" model to the data set. 
...
cv_df %>% 
  ...
  geom_violin() +
  labs(
    title = "Root Mean Square Errors Distributions for Fitted Birth Weight Model",
    x = "Model",
    y = "RMSE"
  ) +
